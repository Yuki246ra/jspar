...
`for()`文を使う方法。ここでは、`assign()`関数を使って、別々の変数にデータを格納します。変数名はもともとのファイル名を使用した。

```{r read_csv_for, exercise=TRUE, exercise.setup = "list_files"}

for (i in 1:length(file.list)) {
  result <- read_csv(file.list[i])
  assign(str_replace(file.list[i], ".csv", ""), result)
}
...
### データフレームの行の並べ替え `order()`

`dplyr::order()`でデータフレームを並べ替える事ができます。
